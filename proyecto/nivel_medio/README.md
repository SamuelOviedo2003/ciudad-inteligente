...
- **Modo nocturno**: bajar ambos potenciómetros de LDR por debajo de `UMBRAL_NOCHE_ENTRA` (800) → los semáforos dejan de ciclar y ambos amarillos parpadean cada 0.5 s, hasta que alguno de los dos supere `UMBRAL_NOCHE_SALE` (1000). La histéresis entre los dos umbrales evita que el ruido del ADC alrededor de un solo valor haga entrar y salir del modo varias veces por segundo (en simulación, con un solo umbral, lo hacía 25 veces en 5 s, reiniciando el ciclo cada vez).
...
- **Polaridad de los botones peatonales**: en Wokwi van a tierra con el pull-up interno (presionado = LOW), y así está `#define P_ACTIVO LOW`. Pero `esp_pruebas.ino`, el código de pruebas del curso, los lee como `INPUT` sin pull-up, lo que sugiere que en la maqueta física pueden estar cableados al revés. Si fuera así y no se corrige, el sistema creería que siempre hay un peatón pidiendo: S1 nunca mostraría más de 2 s de verde y el modo nocturno nunca entraría. **Antes de la demo en hardware real, cargar `esp_pruebas.ino` y anotar qué muestra el LCD para P1/P2 y CNY1..6 en reposo y activados**; con eso se ajustan `P_ACTIVO` y `CNY_ACTIVO` (una línea cada uno). Ahí mismo conviene confirmar si el LCD físico es de 20x4 (como asume el código y el simulador) o de 16x4 (como dice la página del curso): con 16x4 las filas 2 y 3 saldrían corridas.
